...
IDENTIFIER=$(bashio::config 'Inverter_identifier')
...
python3 /config_generator.py --host=$HOST --port=$PORT --model=$MODEL --identifier=$IDENTIFIER --mqtt_host=$MQTT_HOST --mqtt_port=$MQTT_PORT --mqtt_user=$MQTT_USER --mqtt_pass=$MQTT_PASS --scan=$SCAN_INTERVAL --timeout=$SCAN_TIMEOUT --connection=$CONNECTION --meter=$SMART_METER --level=$LEVEL --log_level=$LOG_LEVEL
